...
typedef struct circular_character_list {
  struct circular_character_list *prev;
  struct circular_character_list *next;
  char c;
} c_list;
...
typedef struct pipe_control_block {
  ...
  CondVar hasSpace; /* Condition variable to check if there is space in the
                       buffer */
  CondVar
      hasData; /* Condition variable to check if there are data in the buffer */

  c_list *buffer_node;   /* Node for the data buffer */
  c_list *w_pos, *r_pos; /* Position of the writer and reader */
...
/* @brief Initialize a pipe_cb */
pipe_cb *pipe_init();

/* @brief Write n bytes to a pipe */
int pipe_write(void *pipecb_t, const char *buf, unsigned int n);

/* @brief Read n bytes from a pipe */
int pipe_read(void *pipecb_t, char *buf, unsigned int n);

/* @brief Close operation for the writer */
int pipe_writer_close(void *_pipecb);

/* @brief Close operation for the reader */
int pipe_reader_close(void *_pipecb);

/* @brief Functions that always return error code -1 (for the file_ops) */
...
/*
 Functions for the bounded buffer
 */
...
/**
        ...
        @returns if the reference count is still >0, return 0, else return the
   value returned by the `Close()` operation
*/
