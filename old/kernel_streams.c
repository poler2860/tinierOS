...
int sys_Close(int fd) {
  int retcode =
      (fd >= 0 && fd < MAX_FILEID) ? 0 : -1; /* Closing a closed fd is legal! */
...
unsigned int sys_GetTerminalDevices() { return device_no(DEV_SERIAL); }
...
int sys_OpenNull() { return open_stream(DEV_NULL, 0); }
