...
TCB *spawn_thread(PCB *pcb, void (*func)()) {
  ...
  tcb->priority = 0; /*The new process start in the highest priority queue*/
  tcb->its = QUANTUM;
  tcb->rts = QUANTUM;
...
void create_ptcb(TCB *cur_tcb, Task task, int argl, void *args) {
  ...
  /* Link TCB with PTCB and PCB */
  cur_tcb->ptcb = ptcb;
  ptcb->thread = cur_tcb;

  /*Add the PTCB to the list of CURPROC's PTCB list*/
  rlnode_init(&ptcb->ptcb_node_list, ptcb);
  rlist_push_back(&cur_tcb->owner_pcb->ptcb_list, &ptcb->ptcb_node_list);
}
...
rlnode SCHED[SCHED_QUEUES];        /* The scheduler queues */
rlnode TIMEOUT_LIST;               /* The list of threads with a timeout */
Mutex sched_spinlock = MUTEX_INIT; /* spinlock for scheduler queue */

static int yield_count = 0; /*A yield counter to know when to boost threads */
...
static TCB *sched_queue_select(TCB *current) {
  ...
  for (int i = 0; i < SCHED_QUEUES; i++) {
    /*if the list is not empty get the head and exit the loop*/
  ...
  rlnode *sel = rlist_pop_front(&SCHED[highest_priority]);
...
void sched_boost_priority() {
  ...
  for (int i = 1; i < SCHED_QUEUES; i++) {
    while (!is_rlist_empty(&SCHED[i])) {
      tmp = rlist_pop_front(&SCHED[i]);
      rlist_push_back(&SCHED[i - 1], tmp);
      tmp->tcb->priority--;
    }
  }
}
...
void yield(enum SCHED_CAUSE cause) {
  ...
  TCB *current =
      CURTHREAD; /* Make a local copy of current process, for speed */

  Mutex_Lock(&sched_spinlock);

  /*increase the yield counter*/
  yield_count++;

  /*Check if yield was called enough times to boost all threads */
  if (yield_count == YIELD_RUNS) {
    sched_boost_priority();
    yield_count = 0;
  }
  ...
  case SCHED_IO:
    ...
    break;
  case SCHED_QUANTUM:
    if (current->priority < (SCHED_QUEUES - 1)) {
      current->priority++;
    }
    break;
  case SCHED_MUTEX:
    if (current->curr_cause == current->last_cause &&
        current->priority < (SCHED_QUEUES - 1)) {
      current->priority++;
    }
    break;
  default:
