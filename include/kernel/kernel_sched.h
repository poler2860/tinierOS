...
/**
  ...
  This file contains the definition of the scheduler API, exported to other
...
/** @brief Thread phase.
  ...
  The phase of a thread denotes the state of its context stored in the @c TCB of
  the thread. A @c CTX_CLEAN thread means that, the context stored in the TCB is
  up-to-date. In this case, it is legal to swap context to this thread. A @c
  CTX_DIRTY thread marks the case when the thread was, or still is, being
  executed at some core, therefore its stored context should not be used.

  The following **invariant** of the scheduler guarantees
  correctness:

  > A TCB is in the scheduler
  > queue, if and only if, its @c Thread_state is @c READY and the @c
  Thread_phase > is @c CTX_CLEAN.
...
typedef struct process_thread_control_block {
  struct thread_control_block *thread; /** @brief The thread */
  ...
  int exited;      /** @brief The thread has exited [1,0]*/
  int detached;    /** @brief The thread is detached [1,0]*/
  CondVar exit_cv; /** @brief The exit condition variable */
...
/**
  ...
  Initialize and return a PTCB
 */
void create_ptcb(struct thread_control_block *tcb, Task task, int argl, void *args);
...
typedef struct thread_control_block {
  ...
  int priority; /* @brief The priority queue the thread is currently in*/
  ...
  void (*thread_func)(); /**< @brief The initial function executed by this
                            thread */

  TimerDuration wakeup_time; /**< @brief The time this thread will be woken up
                                by the scheduler */

  rlnode sched_node; /**< @brief Node to use when queueing in the scheduler
                        queue */
  TimerDuration its; /**< @brief Initial time-slice for this thread */
  TimerDuration rts; /**< @brief Remaining time-slice for this thread */

  enum SCHED_CAUSE
      curr_cause; /**< @brief The endcause for the current time-slice */
  enum SCHED_CAUSE
      last_cause; /**< @brief The endcause for the last time-slice */
...
typedef struct core_control_block {
  ...
  TCB *current_thread;  /**< @brief Points to the thread currently owning the
                           core */
  TCB *previous_thread; /**< @brief Points to the thread that previously owned
                           the core */
  TCB idle_thread; /**< @brief Used by the scheduler to handle the core's idle
                      thread */
...
/**
  ...
  a system call can identify the process executing it, and all other
  information.
...
/**
  ...
  This call will change the state of a thread from @c STOPPED or @c INIT (where
  the thread is blocked) to @c READY.
...
/**
  @brief Block the current thread.
    ...
    or @c EXITED. Also, the mutex @c mx, if not `NULL`, will be unlocked,
  atomically with the blocking of the thread.

    In particular, what is meant by 'atomically' is that the thread state will
  change to @c newstate atomically with the mutex unlocking. Note that, the
  state of the current thread is @c RUNNING. Therefore, no other state change
  (such as a wakeup, a yield, another sleep etc) can happen "between" the
  thread's state change and the unlocking.

    If the @c newstate is @c EXITED, the thread will block and also will
  eventually be cleaned-up by the scheduler. Its TCB should not be accessed in
  any way after this call.

    A cause for the sleep must be provided; this parameter indicates to the
  scheduler the source of the sleeping operation, and can be used in scheduler
  heuristics to adjust scheduling decisions.

    A timeout can also be provided. If the timeout is not @c NO_TIMEOUT, then
  the thread will be made ready by the scheduler after the timeout duration has
  passed, even without a call to
    @c wakeup() by another thread.

    @param newstate the new state for the current thread, which must be either
  stopped or exited
    @param mx the mutex to unlock.
    @param cause the cause of the sleep
    @param timeout a timeout for the sleep, or
   */
void sleep_releasing(Thread_state newstate, Mutex *mx, enum SCHED_CAUSE cause,
